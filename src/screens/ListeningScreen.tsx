    ...
    <Screen>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: spacing.xl }}>
        <Card style={{ marginTop: spacing.md, alignItems: "center" }}>
          ...
          <Button title="다시 듣기 Play again" icon="🔊" onPress={() => speak(item.say)} style={{ marginTop: spacing.md, alignSelf: "stretch" }} />
          <Button title="천천히 Slower" icon="🐢" variant="neutral" onPress={() => speak(item.say, { rate: 0.6 })} style={{ marginTop: spacing.sm, alignSelf: "stretch" }} />
        </Card>
        ...
        {picked != null && (
          <View style={{ marginTop: spacing.md }}>
            ...
            {!showTranslation ? (
              <Button title="뜻 보기 Translation" icon="🌐" variant="ghost" onPress={() => setShowTranslation(true)} style={{ marginTop: spacing.sm }} />
            ) : (
              <Card style={{ marginTop: spacing.sm }}>
                <Rom>{item.rom}</Rom>
                <Body style={{ marginTop: 2 }}>{item.en}</Body>
              </Card>
            )}
            <Button title={i + 1 >= listening.length ? "See results" : "다음 Next"} onPress={next} style={{ marginTop: spacing.md }} />
          </View>
        )}
      </ScrollView>
    </Screen>
